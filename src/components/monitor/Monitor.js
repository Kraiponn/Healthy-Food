...
class Monitor extends Component {
    ...
    delOrder(product){
        ...
        let resOrder = this.state.orders.filter(order => 
                order.product.id !== product.id);
        const totalPrice = this.state.totalPrice - (findOrder.quantity * findOrder.product.unitPrice);
        this.setState({totalPrice: totalPrice, orders: resOrder, confirm: true, msg: 'ลบรายการเรียบร้อย'});
    }
    ...
    confirmOrder(){
        ...
        Axios.post("http://localhost:3010/orders", 
            { orderedDate: new Date(), totalPrice, orders })
            .then(res => {
                this.setState({totalPrice: 0, orders: [], confirm: true, msg: 'เพิ่มข้อมูลเรียบร้อย'})
            });
    }
    ...
    render(){
        return (
            <div className="container-fluid">
                {this.state.confirm &&
                    <div className="alert alert-success title text-right">
                        {this.state.msg}
                    </div>
                }
                <div className="row">
